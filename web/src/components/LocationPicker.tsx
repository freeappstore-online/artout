...
export function LocationPill({ currentPath, count, onOpen }: { currentPath: string | null; count: number; onOpen: () => void }) {
  ...
  return (
    <button
      onClick={onOpen}
      className="flex items-center gap-1.5 whitespace-nowrap rounded-full border border-[var(--line-strong)] bg-[var(--panel)] px-3 py-1.5 text-xs font-semibold text-[var(--ink)] backdrop-blur"
    >
      ...
      <svg className="h-3 w-3 shrink-0 text-[var(--muted)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
        <path d="M6 9l6 6 6-6" />
      </svg>
    </button>
  )
